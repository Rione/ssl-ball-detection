    ...
    for path in paths_to_try:
        if os.path.exists(path):
            ...
            try:
                ...
                if "minThreshold" in settings and isinstance(settings["minThreshold"], list):
                    settings["minThreshold"] = np.array(settings["minThreshold"], dtype=np.uint8) # Use uint8 for HSV
                if "maxThreshold" in settings and isinstance(settings["maxThreshold"], list):
                    settings["maxThreshold"] = np.array(settings["maxThreshold"], dtype=np.uint8) # Use uint8 for HSV

                # Ensure numeric types for others if needed (json loads them as int/float)
                if "ballDetectRadius" in settings:
                    settings["ballDetectRadius"] = int(settings["ballDetectRadius"]) # Ensure int if needed
        ...
        print(f"Warning: Settings file '{file_path}' not found in checked locations. Using default values.")
...
class ImageProcessor:
    def __init__(self, settings=None):
        ...
        self._minThreshold = settings.get("minThreshold", np.array([1, 120, 100], dtype=np.uint8))
        self._maxThreshold = settings.get("maxThreshold", np.array([15, 255, 255], dtype=np.uint8))
        self._ksize = tuple(settings.get("gaussianKernelSize", (5, 5))) # Example: allow configuring ksize via JSON
        self._sigmaX = settings.get("gaussianSigmaX", 0)
        self._shape = cv2.MORPH_RECT # Could also be made configurable
        self._size = tuple(settings.get("morphKernelSize", (3, 3))) # Example: allow configuring morph size
...
class BallDetector:
    def __init__(self, settings=None):
        ...
        self._circularityThreshold = float(settings.get("circularityThreshold", 0.2)) # Ensure float
        self._minContourArea = float(settings.get("minContourArea", 100)) # Example: add min area threshold
    ...
    def detect(self, frame):
        ...
        valid_contours = [cnt for cnt in contours if cv2.contourArea(cnt) > self._minContourArea]
        ...
        if self._isCircular(bestContour):
            ...
            circleContour = self._createCircleContour(x + offset[0], y + offset[1], radius)
    ...
    def _focus(self, frame, center):
        ...
        if yMin >= yMax or xMin >= xMax:
             print(f"Warning: Invalid ROI calculated: ({xMin},{yMin}) to ({xMax},{yMax}). Using full frame.")
             xMin, yMin, xMax, yMax = 0, 0, width, height # Fallback to full frame

        roi = frame[yMin:yMax, xMin:xMax]
        offset = (xMin, yMin) # Top-left corner of the ROI in original frame coordinates
...
class Visualizer:
    ...
    def draw(self, frame, center, circleContour, vertices):
        ...
        if vertices:
            cv2.rectangle(frame, (vertices[0], vertices[1]), (vertices[2], vertices[3]), (0, 0, 255), 1) # Red ROI box

        # Draw detected circle contour
        if circleContour is not None:
            cv2.drawContours(frame, [circleContour], -1, (255, 0, 0), 2) # Blue circle outline
...
class VideoCapture:
    def __init__(self, device=0, settings=None):
        ...
        self._width = int(settings.get("frameWidth", 640)) # Example: Add configurable width
        self._height = int(settings.get("frameHeight", 480)) # Example: Add configurable height
...
def main():
    ...
    try:
        ...
        output_height = int(settings.get("outputFrameHeight", 96)) # Configurable output size
